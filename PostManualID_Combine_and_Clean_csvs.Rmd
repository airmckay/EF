...
### Overview of equipment failures 
...
```{r Site column}
# Split recorder serial number from IN.FILE column
IDtest3 <- IDtest2 %>% separate(IN.FILE, into = c("det.nr", "other.values"), sep = "_", extra = "merge")

# Trim down to just essential columns
IDtest4 <- IDtest3 %>% 
  select(Filename, det.nr, DURATION, 
         DATE, TIME, HOUR, 
         DATE.12, TIME.12, HOUR.12, 
...
# Load data with site and detector number
site <- read_excel("~/1. PhD_Main/GitHub_link/EcoForest/Data wrangling Emma/2. Cleaning data/Input/Site_DetectorNr.xlsx")

# Merge data

IDtest5 <- merge(IDtest4, site, by = "det.nr")

IDtest5$Site <- factor(IDtest5$Site)
...
# Saving csv
setwd(output)
#write.csv(IDtest5, file="cleanedID_05.02.24.csv", na="NA")
...
## Why IDtest2 here instead of IDtest5? 
...
levels(bats1$taxa) <- list("Eptesicus nilssonii" = "EPNI",
                           "Myotis" = "MYOT",
                           ...
                           "blank" = "") 
summary(bats1$taxa)
  # Eptesicus nilssonii                Myotis      Nyctalus noctula 
  #                4200                  4481                    15 
  # Vespertilio murinus Pipistrellus pygmaeus           Unknown bat 
  #                   8                     2                   205 
  #               blank                  NA's 
  #                   3                    15 

## 15 NAs which were not parsed properly. "EPNINoID"
## Ask Emma if she also had this issue, if so it should be fixed in an early stage (during manual cleaning) 
# I will do a temporary solution here. 

bats1.5 <- bats1 %>% 
  mutate(taxa1 = factor(case_when(
  MANUAL.ID == "EPNINoID" ~ "EPNI_NoID",
  .default = taxa))) %>% 
  separate_rows(taxa1, sep = "_") %>% 
  mutate(taxa1 = factor(taxa1))

summary(bats1.5$taxa1) 

bats1.5$taxa2 <- gsub("EPNI", "Eptesicus nilssonii", bats1.5$taxa1)

bats1.5$taxa3 <- factor(gsub("NoID", "Unknown bat", bats1.5$taxa2))

table(bats1.5$taxa3)
  # Eptesicus nilssonii                Myotis 
  #                4215                  4481 
  #    Nyctalus noctula Pipistrellus pygmaeus 
  #                  15                     2 
  #         Unknown bat   Vespertilio murinus 
  #                 220                     8 
  #               blank 
  #                   3
names(bats1.5)

bats1 <- bats1.5 %>% select(-c(taxa, taxa1, taxa2)) %>% rename(taxa = taxa3)
summary(bats1)
dim(bats1)
# 8944   16
```
...
## Note that you have 8944 rows here which you should have 8944 bat passes in your aggregated dataset as well. 

bat_sitemap <- bats1 %>% select(Site) %>% distinct() 

# You had the total number of bat passes per site but I think you should do this for total taxa per site. 
...
summary(bats_nights_sum1$batpass)
   ...
   # 1.00    1.00    3.00   11.93    9.00  261.00 
## Note that the highest number of bat passes in a night that you have is 261.00, so that should also be the same at the end. 
...
temp2 <- merge(temp, active.nights1) # 5355 obs of 9 variables 

summary(temp2$Site) # number of monitoring nights per site with zero nights included
# EF11-CC-B EF11-NN-B EF12-CC-B EF12-NN-B  EF2-CC-B  EF2-NN-B  EF3-CC-B 
#       301       301       140       294       203       294       287 
#  EF3-NN-B  EF4-CC-B  EF4-NN-B  EF5-CC-B  EF5-NN-B  EF6-CC-B  EF6-NN-B 
#       287       294       294       287       287       294       294 
#  EF7-NN-B  EF8-CC-B  EF8-NN-B  EF9-CC-B  EF9-NN-B 
#       294       301       301       301       301 

temp3 <- merge(temp2, dat, all.x = TRUE) 
# 5355 obs of 9 vars 

summary(temp3$Site)# number of nights per taxa per site plot 
...
# Trim down to just essential columns
nbats2 <- nbats1 %>% 
  select(Site, night, batpass, taxa, 
         LOCATION, SITETYPE, active.night) %>% distinct() 
## Make sure you use distinct here to remove duplicate rows. 
view(nbats2)

sum(nbats2$batpass)
# 17817
## This number should be 8944. 
17817/8944
# 1.992062
# For some reason this is doubled. 

## 
# 8944

setwd(output)
#write.csv(nbats2, "aggregatedbynight_totalbats_zeroinserted.csv") 
...
```{r Load csv}
big_ID <- read.csv("combinedID_01.02.24.csv")
```
...
# Load data with site and detector number
site <- read_excel("C:/Users/emmah/OneDrive/Downloads/Master/Data/Site_DetectorNr.xlsx")
